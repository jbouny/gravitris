...
{
	this.m_CubeGeometry = new THREE.CubeGeometry( 1, 1, 1 );
...
PoolBlocks3D.prototype.Block = function( inBlock )
{
	...
	aCube.position.set( inBlock.m_X - Config.ms_GameWidth / 2 + 0.5, Config.ms_GameHeight - inBlock.m_Y, inBlock.m_Z );
	aCube.rotation.set( 0, 0, -inBlock.m_Rotation );
	return aCube;
};
...
var Display3D =
{
	...
	Initialize: function( inIdCanvas )
	{
		...
		this.ms_Camera.lookAt( new THREE.Vector3( 0, Config.ms_GameHeight * 0.415, 0 ) );
		this.ms_Scene = new THREE.Scene() ;
	
		// Add light
		this.ms_Scene.add( this.CreatePointLight( 0xffff55, 15, 4, 6 ) );
		this.ms_Scene.add( this.CreatePointLight( 0x55ffff, -15, 4, 6 ) );
		this.ms_Scene.add( this.CreatePointLight( 0xff55ff, 15, -10, 6 ) );
		this.ms_Scene.add( this.CreatePointLight( 0xffffff, 10, 30, 10 ) );
		
		// Contour of the game
		var aLines = new THREE.Geometry();
		aLines.vertices.push( new THREE.Vector3( Config.ms_GameWidth / 2, 0.67 * Config.ms_GameHeight, 0 ) );
		aLines.vertices.push( new THREE.Vector3( Config.ms_GameWidth / 2, 0.5, 0 ) );
		aLines.vertices.push( new THREE.Vector3( - Config.ms_GameWidth / 2, 0.5, 0 ) );
		aLines.vertices.push( new THREE.Vector3( - Config.ms_GameWidth / 2, 0.67 * Config.ms_GameHeight, 0 ) );
	...
	Display: function()
	{
		if( !this.ms_IsDisplaying )
		{
			...
			for( var i = 0; i < Game.ms_Blocks.length; ++i ) 
				aGroup.add( this.ms_PoolBlocks.Block( Game.ms_Blocks[i] ) );
			
			// Pause or game over
			if( Game.ms_IsPause )
				aGroup.add( this.ms_Texts["Pause"] );
